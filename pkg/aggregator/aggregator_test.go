...
func Test_runOnce(t *testing.T) {
	...
	assert.Equal(t, 3, len(c.Configs[0].ApplicationConfig))
	assert.Equal(t, 3, len(c.Configs[1].ApplicationConfig))
	mc0, ok := c.Configs[0].ApplicationConfig["metric_configs"].([]interface{})
	assert.True(t, ok)
	assert.Equal(t, 2, len(mc0))
	mc1, ok := c.Configs[1].ApplicationConfig["metric_configs"].([]interface{})
	assert.True(t, ok)
	assert.Equal(t, 2, len(mc1))
	uc0, ok := c.Configs[0].ApplicationConfig["unified_configs"].([]interface{})
	assert.True(t, ok)
	assert.Equal(t, 2, len(uc0))
	uc1, ok := c.Configs[1].ApplicationConfig["unified_configs"].([]interface{})
	assert.True(t, ok)
